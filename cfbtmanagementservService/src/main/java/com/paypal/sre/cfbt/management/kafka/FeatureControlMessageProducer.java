 ...
 */
@Component
public class FeatureControlMessageProducer {

    private static Producer<String, FeatureControlMessage> producer;
    private static final org.slf4j.Logger LOGGER = LoggerFactory.getLogger(FeatureControlMessageProducer.class);
    public static final String VALUE_SERIALIZER = "com.paypal.sre.cfbt.management.kafka.FeatureControlMessageSerializer";
    ...
    public static final String CLIENT_ID = "feature-control-message-producer";
    
    private static String featureControlTopics = null;

    @Inject
    private FeatureControlMessageProducer(Configuration config) {

        Properties props = getProducerProperties(config);
        ...
        featureControlTopics = config.getString(Topic.Feature_Control_Events.getProperty(),
                Topic.Feature_Control_Events.getName());
        ...
        KafkaConfigServiceInput kafkaConfigServiceInput = new KafkaConfigServiceInputRaptorBuilder()
                .withClientId(clientId).withClientRole(clientRole).withClientEnv(clientEnv).withTopics(featureControlTopics)
        ...
        producer = new KafkaProducer<String, FeatureControlMessage>(props);
    }

    /**
     * @return the Kafka {@link Producer} for {@link ExecutorControlMessage}
     */
    public synchronized static Producer<String, FeatureControlMessage> getProducer() {
        return producer;
    }
     ...
     */
    private Properties getProducerProperties(Configuration config) {
    ...
    public static void publish(FeatureControlMessage FeatureControlControlMessage) {
        FeatureControlMessageProducer.getProducer().send(new ProducerRecord<String, FeatureControlMessage>(
                featureControlTopics, FeatureControlControlMessage));
    }
}
